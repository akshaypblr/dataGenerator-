...
original_author="akshaypblr"
original_name="datagenerator_"
original_urlname="dataGenerator-"
original_description="Awesome datagenerator_ created by akshaypblr" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/datagenerator_ "${name}"
...
then
    ...
    echo "then run 'datagenerator_' or 'python -m datagenerator_'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'datagenerator_' or 'python -m datagenerator_' or 'poetry run datagenerator_'"
fi
